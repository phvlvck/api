...
// ============================================
// الخادم الحقيقي - Netlify Functions
// مع Supabase PostgreSQL
// ============================================
...
const handlers = {
    ...
    async login(event) {
        ...
        if (error || !user) {
            await logEvent('login_failed', `فشل تسجيل الدخول: ${email}`);
        ...
        if (!validPassword) {
            await logEvent('login_failed', `كلمة مرور خاطئة: ${email}`);
            return { error: 'Invalid credentials', status: 401 };
        }
        ...
        await logEvent('login_success', `تسجيل دخول: ${email}`, user.id);
    ...
    async createSubscription(event) {
        ...
        if (error) {
            await logEvent('subscription_error', `فشل إنشاء اشتراك: ${error.message}`, auth.user.id, 'error');
            return { error: 'Failed to create subscription', status: 500 };
        }
        
        await logEvent('subscription_created', `تم إنشاء اشتراك: ${licenseKey}`, auth.user.id);
    ...
    async verifySubscription(event) {
        ...
        if (count >= subscription.max_devices) {
            ...
            if (!existingDevice) {
                await logEvent('subscription_limit', `الحد الأقصى للأجهزة: ${licenseKey}`);
        ...
        await logEvent('subscription_verified', `تم التحقق: ${licenseKey}`, subscription.user_id);
    ...
    async getStats(event) {
        ...
        return {
            ...
            bannedDevices: bannedDevices.count || 0,
            expiringSoon: expiringSoon.count || 0,
            apiCalls: Math.floor(Math.random() * 1000) + 100 // وهمي
        };
    },

    // ============================================
    // POST /heartbeat - نبض القلب
    // ============================================
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        switch (true) {
            ...
            case path === '/heartbeat' && method === 'POST':
                result = await handlers.heartbeat(event);
                break;
